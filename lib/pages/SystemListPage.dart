...
import 'package:flutter_demo/bean/article_list_bean.dart';
...
//  int cid=0;
  ...
  Widget build(BuildContext context) {
     return new DefaultTabController(length: children.length, child: Scaffold(
       appBar: AppBar(
         title: Text(titleName),
         bottom: new TabBar(
           isScrollable: true,
           tabs: children.map((Children child) {
             return new Tab(
               text: child.name,
             );
           }).toList(),
...
// 底部刷新
  ...
  void getPageData() async {
    var response = await new HttpUtil().get(Api.KNOWLEDGE_LIST +
        ...
        cid.toString());
    print(response);
    var item = new ArticleListBean.fromJson(response);
    setState(() {
      isLoading = false;
      if (pageIndex == 0) {
